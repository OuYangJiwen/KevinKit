...
inline CENTER CENTERMake(float x,float y){
    CENTER center = {x,y};
    return center;
}
...
RGBType HSV_to_RGB( HSVType HSV )
{
 ...
 POINT center_point_alue(RGBType rgb,CENTER center){
   ...
   CENTER x_center = CENTERMake(1, 1);
   HSVType currentHSVWithNew= RGB_to_HSV(rgb);
    currentHSVWithNew.v=1.0;
    double angle=currentHSVWithNew.h*2.0*M_PI;
    double radius=center.x- x_center.x/2;
    ...
    float y=center.y-sinf(angle)*radius;
     ...
     x = round(x - x_center.x) + x_center.x;
     y = round(y - x_center.y) + x_center.y;
